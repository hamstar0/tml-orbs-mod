	...
	partial class OrbsNPC : GlobalNPC {
		private void TricksterModLoot( NPC npc ) {
			...
			if( !tricksterNpc.HasAttacked ) {
				int itemWho = Item.NewItem( npc.getRect(), ModContent.ItemType<PinkOrbItem>(), OrbsConfig.Instance.PinkOrbDropsViaTrickster );
		...
		private void StaffOfGaiaModLoot( NPC npc ) {
			...
			if( !hasKills ) {
				int itemWho = Item.NewItem( npc.getRect(), ModContent.ItemType<RedOrbItem>(), OrbsConfig.Instance.RedOrbDropsViaNonSoGBoss );
				if( Main.netMode == 2 ) {
					NetMessage.SendData( MessageID.SyncItem, -1, -1, null, itemWho );
				}
			}
